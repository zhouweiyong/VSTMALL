...
public class ImageTools {
	...
	public static final int CAPTURE_IMG_WIDTH = 600;// 裁切大小
	public static final int CAPTURE_IMG_HEIGHT= 338;// 裁切大小
	
	public static final int USER_IMG_WIDTH = 200;// 裁切大小
	public static final int USER_IMG_HEIGHT= 200;// 裁切大小
...
//		intent.setType("image/*");
		...
		intent.putExtra("outputX", USER_IMG_WIDTH);
		intent.putExtra("outputY", USER_IMG_HEIGHT);
		intent.putExtra(MediaStore.EXTRA_OUTPUT, photoUri);
	...
	public static Intent cropPhotoOfCompressFromGalleryIntent1(Uri photoUri) {
		// Intent intent = new Intent("com.android.camera.action.CROP");
		// intent.setDataAndType(photoUri, "image/*");
		Intent intent = new Intent("android.intent.action.PICK");
		intent.setDataAndType(Media.INTERNAL_CONTENT_URI, "image/*");
//		intent.setType("image/*");
	 ...
	 * 图库选择图片、剪裁图片并压缩，和拍照回调可做整合 一般用于获取大图；Uri方式 无法加载预览图片；
	 */
	public static Intent cropPhotoOfCompressFromGalleryIntent2(Uri photoUri) {
		// Intent intent = new Intent("com.android.camera.action.CROP");
		// intent.setDataAndType(photoUri, "image/*");
		Intent intent = new Intent("android.intent.action.PICK");
		intent.setDataAndType(Media.INTERNAL_CONTENT_URI, "image/*");
	...
	public static Intent cropPhotoOfCompressIntent(Uri photoUri) {
		...
		intent.putExtra("aspectX", 1);
		intent.putExtra("aspectY", 1);
		intent.putExtra("outputX", USER_IMG_HEIGHT);
		intent.putExtra("outputY", USER_IMG_WIDTH);
		intent.putExtra("scale", true);
		intent.putExtra(MediaStore.EXTRA_OUTPUT, photoUri);
		intent.putExtra("return-data", false);
		intent.putExtra("outputFormat", Bitmap.CompressFormat.JPEG.toString());
...
//		intent.setAction(Intent.ACTION_GET_CONTENT);
	...
	public  static File initTempFile(Context context) {
		File uploadFileDir = new File(new File(context.getCacheDir(),"vst"), "img_temp");//按照volly写的
